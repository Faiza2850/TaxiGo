...
func Dump() error {
	...
	args := []string{"-s", "-d", config.Cfg.PG_DB, "-w"}
	...
	if config.Cfg.PG_HOST != "" {
		args = append(args, "-h", config.Cfg.PG_HOST)
	}
	port, err := strconv.Atoi(strconv.Itoa(config.Cfg.PG_PORT))
	if err != nil {
		return err
	}
	if port != 0 {
	...
	if config.Cfg.PG_USER != "" {
		args = append(args, "-U", config.Cfg.PG_USER)
	}
	cmd := exec.Command(program, args...)
	if config.Cfg.PG_PASSWORD != "" {
		cmd.Env = append(os.Environ(), "PGPASSWORD="+config.Cfg.PG_PASSWORD)
	}
	buf := &bytes.Buffer{}
	cmd.Stdout = buf
	err = cmd.Run()
